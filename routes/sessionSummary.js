...
router.post("/", async (req, res) => {
  ...
  // Also pull the student's email if this session has one on file (set
  // for domain-based permanent-access sessions) — this way the frontend
  // never has to re-send it separately.
  let sessionId = null;
  let sessionEmail = null;
  if (sessionToken) {
    try {
      const lookup = await query(
        `SELECT id, email FROM user_sessions WHERE session_token = $1 LIMIT 1`,
        [sessionToken],
      );
      sessionId = lookup.rows[0]?.id || null;
      sessionEmail = lookup.rows[0]?.email || null;
      if (!sessionId) {
        console.warn(
  ...
  try {
    await query(
      `INSERT INTO session_summaries (session_id, access_code, email, topic, summary, message_count)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [sessionId, accessCode || null, sessionEmail, topic, summary, conversationLog.length],
    );
...
router.get("/recent", authenticate, async (req, res) => {
  ...
  try {
    const result = await query(
      `SELECT id, session_id, access_code, email, topic, summary, message_count, created_at
       FROM session_summaries
