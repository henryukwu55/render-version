...
const state = {
  ...
  isMicOn: false,
  accessCode: null,
  isPermanent: false, // true for domain-based email access — no timer, no expiry
  // Transcript
...
function init() {
  ...
  emailInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter") submitStudentEmail();
  });
  submitEmailBtn.addEventListener("click", submitStudentEmail);
...
// ── Validate Email (domain allowlist — permanent, no timer) ───────
async function submitStudentEmail() {
  const email = emailInput.value.trim();
  codeError.textContent = "";
  if (!email || !email.includes("@")) {
    codeError.textContent = "Please enter a valid email address.";
    return;
  }

  submitEmailBtn.disabled = true;
  submitEmailBtn.textContent = "Checking…";
  try {
    const res = await fetch("/api/codes/email-access", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const data = await res.json();
    if (!res.ok) {
      codeError.textContent = data.error || "That email isn't on the access list.";
      submitEmailBtn.disabled = false;
      submitEmailBtn.textContent = "Continue with Email";
      return;
    }
    state.sessionToken = data.session_token;
    ...
    openAppScreen("Unlimited session started. Click \"Connect to Mentor\" to begin.");
  } catch (err) {
    codeError.textContent = "Network error. Please try again.";
    submitEmailBtn.disabled = false;
    submitEmailBtn.textContent = "Continue with Email";
...
// ── Shared transition from code-entry screen to the app screen ────
function openAppScreen(systemNote) {
  ...
  startTimer();
  if (systemNote) addSystemNote(systemNote);
}

// ── Timer ─────────────────────────────────────────────────────────
function startTimer() {
  if (state.isPermanent) {
    // Unlimited access — no countdown, no expiry
